...
No entanto, em um menu vertical (barra lateral), geralmente é desejável deixar um submenu aberto quando ele contém o item de menu ativo. No Joomla 6.0, uma nova classe CSS, `nav-active-open`, foi introduzida especificamente para permitir o controle sobre se os submenus serão abertos automaticamente ao carregar a página para o item de menu ativo. A definição dessa classe agora torna isso possível. A classe é definida no módulo por meio do painel administrativo.
...
Se quiser manter todos os submenus abertos, não será necessário usar uma alternância suspensa. Em vez disso, use uma [substituição de template](jdocmanual?article=user/templates/template-overrides).

Veja como essa substituição específica de template é criada:
...
Isso copiará todos os arquivos de layout do menu do módulo de menu para a substituição. Em seguida, ele retornará à aba Editor.
